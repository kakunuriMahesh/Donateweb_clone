        ...
        $("body").on("click", ".wpdevart-submit-update", function(e) {
            ...
            if (wpdevart_required_field === true) {
                ...
                $.post(wpdevart.ajaxUrl, {
                    ...
                    $(window).scrollTo(reserv_cont, 400, {
                        'offset': {
                            'top': -(offset)
                        }
                    });
                    ...
                    /*Max day/hour count*/
                        ...
                        if (av_min <= 0) {
                            ...
                            if (hours_enabled && hour == true) {
                                ...
                                $(window).scrollTo($(form_container), 400, {
                                    'offset': {
                                        'top': -(offset)
                                    }
                                });
                                ...
                                if (!edit) {
                                    $.get(wpdevart.ajaxUrl, {
                                        ...
                                        if (date_data == 0) {
                                            ...
                                            $(window).scrollTo($(div_container).find(".error_text_container"), 400, {
                                                'offset': {
                                                    'top': -(offset)
                                                }
                                            });
...
function reservationError(div_container, id, item, selected, offset, type) {
    ...
    jQuery(window).scrollTo(jQuery(div_container).find(".error_text_container"), 400, {
        'offset': {
            'top': -(offset)
        }
    });
    ...
    if (jQuery(submit).closest("form").find(".wpdevart-required:not(span)").length != 0 || jQuery(submit).closest("form").find(".wpdevart-email").length != 0) {
        jQuery(submit).closest("form").find(".wpdevart-required:not(span),.wpdevart-email").each(function(index, el) {
            ...
            if (error === true) {
                ...
                jQuery(el).focus();
                jQuery(window).scrollTo(jQuery(el), 400, {
                    'offset': {
                        'top': -(offset)
                    }
                });
...
function wpdevartec_submit(el, booking_id) {
    ...
    jQuery.each(el.closest("form").find("input[type=file]"), function() {
        ...
        if (error > 0) {
            jQuery(this).focus();
            jQuery(window).scrollTo(jQuery(this), 400, {
                'offset': {
                    'top': -(offset)
                }
            });
    ...
    jQuery.ajax({
        ...
        success: function(data) {
            ...
            jQuery(window).scrollTo(jQuery(reserv_cont).find('div.successfully_text_container'), 400, {
                'offset': {
                    'top': -(wpdevartBooking(booking_id, "offset"))
                }
            });
        },
