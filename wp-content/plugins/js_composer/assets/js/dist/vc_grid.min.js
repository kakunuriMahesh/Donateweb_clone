...
! function($) {
    ...
    (vcGridStyleAll = function(grid) {
        ...
        this.$el = this.grid.$el, this.$content = this.$el, this.$content.find(".vc_grid-item").length ? (this.grid.initFilter(), this.filter(), this.showItems(), this.filterValue = -1, window.vc_prettyPhoto()) : (_.defer(this.setIsLoading), this.grid.ajax({}, this.addItems))
        ...
        window.vcGridSettings.addItemsAnimation = animation, this.$content.find(".vc_visible-item").removeClass("vc_visible-item " + window.vcGridSettings.addItemsAnimation + " animated"), this.filterValue = filter, _.defer(this.showItems)
    }, vcGridStyleAll.prototype.showItems = function() {
        var $els = this.$content.find(".vc_grid-item" + this.filterValue);
        this.setIsLoading();
        var animation = this.$content.closest(".vc_grid-container").data("initial-loading-animation");
        window.vcGridSettings.addItemsAnimation = animation, $els.addClass("vc_visible-item " + ("none" !== window.vcGridSettings.addItemsAnimation ? window.vcGridSettings.addItemsAnimation + " animated" : "")), this.unsetIsLoading(), $(window).trigger("grid:items:added", this.$el)
...
function($) {
    ...
    (vcGridStyleLoadMore = function(grid) {
        ...
        this.$el = this.grid.$el, this.$content = this.$el, this.grid.initFilter(), this.filter(), this.showItems(), this.filterValue = -1, window.vc_prettyPhoto(), this.$content.append(this.$loader), this.grid.ajax({}, this.addItems)
    }, vcGridStyleLoadMore.prototype.showItems = function() {
        var $els = this.$content.find(".vc_grid_filter-item:not(.vc_visible-item):lt(" + this.settings.items_per_page + ")");
        this.setIsLoading();
        var animation = this.$content.closest(".vc_grid-container").data("initial-loading-animation");
        window.vcGridSettings.addItemsAnimation = animation, $els.addClass("vc_visible-item " + window.vcGridSettings.addItemsAnimation + " animated"), this.unsetIsLoading(), $(window).trigger("grid:items:added", this.$el)
        ...
        if (html && html.length) {
            var $els = $(html);
            this.$el.html($els), this.unsetIsLoading(), this.$content = $els.find('[data-vc-grid-content="true"]'), this.$loadMoreBtn = $els.find('[data-vc-grid-load-more-btn="true"] .vc_btn3'), this.$loadMoreBtn.length || (this.$loadMoreBtn = $els.find('[data-vc-grid-load-more-btn="true"] .vc_btn'));
            var self = this;
...
function($) {
    ...
    ! function($) {
        ...
        window.VcGridInfinite = function($el, options) {
            ...
            }, options)).element = $el, opts.handler = function(direction) {
    ...
    }(window.jQuery), (vcGridStyleLazy = function(grid) {
        ...
        var self = this;
        this.$content.unbind(".vcwaypoint"), new window.VcGridInfinite(this.$content, {
            container: "auto",
            items: ".vc_grid-item",
            handle: {
                load: function(opts) {
...
function($) {
    ...
    (vcGridStylePagination = function(grid) {
        this.grid = grid, this.settings = grid.settings, this.$el = !1, this.$content = !1, this.filterValue = null, this.isLoading = !1, this.htmlCache = !1, this.$loader = $('<div class="vc_grid-loading"></div>'), this.init()
    }).prototype.init = function() {
        ...
        return filter = _.isUndefined(filter) || "*" === filter ? "" : filter, this.filterValue != filter && (!(!this.htmlCache || !this.htmlCache.length) && (this.$content.data("owl.vccarousel") && (this.$content.off("initialized.owl.vccarousel"), this.$content.off("changed.owl.vccarousel"), this.$content.data("vcPagination") && this.$content.data("vcPagination").twbsPagination("destroy"), this.$content.data("owl.vccarousel").destroy()), this.$content.empty(), $html = $(".vc_grid-item", this.htmlCache), "" !== filter && ($html = $html.filter(filter)), this.filterValue = filter, void this.buildSlides($html.addClass("vc_visible-item"))));
        var $html
        ...
        if ($.fn.vcOwlCarousel) {
            ...
            ($vcCarousel = this.$content.data("owl.vccarousel")) && $vcCarousel.destroy(), this.$content.on("initialized.owl.vccarousel", function(event) {
...
function($) {
    ...
    (vcGridStyleAllMasonry = function(grid) {
        ...
        setMasonry: function() {
            ...
            window.innerWidth < window.vcGridSettings.mobileWindowWidth ? (this.$content.data("masonry") && this.$content.masonry("destroy"), this.masonryEnabled = !1) : this.masonryEnabled ? (this.$content.masonry("reloadItems"), this.$content.masonry("layout")) : (settings = {
...
function($) {
    ...
    (VcGrid = function(el) {
        ...
        this.$filter = this.$el.find("[data-vc-grid-filter]"), this.$filterDropdown = this.$el.find("[data-vc-grid-filter-select]"), this.$filter.length && this.$filter.find(".vc_grid-filter-item").off("click.vc-grid").on("click.vc-grid", this.filterItems), this.$filterDropdown.length && this.$filterDropdown.off("change.vc-grid").on("change.vc-grid", this.filterItemsDropdown)
        ...
        window.vc_iframe ? (this.$el.off("click.zonePostLink").on("click.zonePostLink", "[data-vc-link]", function() {
            ...
            if (e.preventDefault(), $this.hasClass("vc-gitem-link-ajax")) {
                ...
                httpRequest.onreadystatechange = function() {
                    XMLHttpRequest.DONE === httpRequest.readyState && (200 === httpRequest.status ? $this.addClass("vc-spinner-complete") : $this.addClass("vc-spinner-failed"), timeout && (window.clearTimeout(timeout), timeout = 0), timeout = window.setTimeout(removeSpinner, 500))
                }
        ...
        var $control, filter;
        if (e && e.preventDefault && e.preventDefault(), this.style ? $.camelCase("filter-" + this.style) : "filterAll", filter = ($control = $(e.currentTarget).find("[data-vc-grid-filter-value]")).data("vcGridFilterValue"), $control.hasClass("vc_active")) return !1;
    ...
    }, $.fn.vcGrid.Constructor = VcGrid, $(document).ready(function() {
        $("[data-vc-grid-settings]").vcGrid()
    }), window.VcGrid = VcGrid, window.vcGridSettings = vcGridSettings
}(window.jQuery);